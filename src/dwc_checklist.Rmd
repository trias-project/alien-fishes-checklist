...
`eventDate` will be of the format `start_year` / `end_year` (yyyy/yyyy).
`start_year`(yyyy) will contain the information from the first observation (`raw_introduction`). For the `end_year`, we have to assume when the presence of the species was last verified. We will use the publication year of the paper (`2018`). 
