...
output:
  html_document:
    ...
    theme: yeti
    df_print: paged
knit: (function(input_file, encoding) { rmarkdown::render(input_file, encoding = enstqcoding, output_file = paste0("../docs/",sub(".Rmd", ".html", basename(input_file))))})
---
...
In the raw data file, both `A` and `A*` stand for `acclimatized`, even for species (like Nile tilapia) in secluded warmer water of power plants. `N` refers to `naturalized`. For the TrIAS project, we decided to use the vocabulary for `invasion stage` contained in the unified framework for biological invasions of [Blackburn et al. 2011](http://doc.rero.ch/record/24725/files/bach_puf.pdf). `Acclimatized` in the raw data file refers to the status where the alien species has overcome the geographical barrier, the captivity/cultivation barrier and the survival barier, but where it fails to establish because individuals in the population fail to reproduce. In Blackburn et al. (2011), this corresponds to the term `casual`. We discard the term `naturalized` used in the raw data file, because often, there's no sensible criterium to distinguish between casual/naturalised of naturalised/established. Thus, here, we consider all `naturalized` species to be `established`.
