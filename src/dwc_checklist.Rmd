...
In the description extension we want to include native range (`raw_origin`) and pathway (`raw_pathway_s`) information. We'll create a separate data frame for both and then combine these with union.
...
`Pathway` information was already generated for `establishmentMeans` in the Distribution Extension and saved in a dataframe `pathway`:

```{r}
head(pathway)
```

It contains one record per pathway (with potentially more than one pathway per taxon). First, we change column name `mapped_value` to description:
...
### Union native range and pathway:

```{r}
description_ext <- bind_rows(native_range, pathway)
```
