...
The `eventDate` will be of the format `start_year`/`end_year` (`yyyy/yyyy`). The `start_year` will contain the information from the first observation (`raw_introduction`). For the `end_year`, we have to assume when the presence of the species was last verified (even though all species are considered established). We will use the publication year of the Verreycken et al. 2018.
...
Both `A` and `A*` stand for `established`, even for species (like Nile tilapia) in secluded warmer water of power plants. `N` refers to `naturalized`.

However, for the TrIAS project, we decided to use the unified framework for biological invasions of [Blackburn et al. 2011](http://doc.rero.ch/record/24725/files/bach_puf.pdf), but we discard the terms `naturalized` (because often, there's no sensible criterium to distinguish between casual/naturalised of naturalised/established) and `invasive` (this is a label that should only be put on a species after risk assessment). Thus, here, we consider all species as `established`.
